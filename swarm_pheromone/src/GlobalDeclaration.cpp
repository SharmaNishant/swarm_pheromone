...
bool isInsideBoundary(Point location) {
  ...
  bool result =
      isInsideRectangle(boundaryOne, boundaryTwo, boundaryThree, location);

  return result;
}

bool isInsideRectangle(Point boundaryOne, Point boundaryTwo,
                       Point boundaryThree, Point location) {
  float firstBoundary, secondBoundary;

  firstBoundary = pow((boundaryThree.x - boundaryOne.x), 2) +
                  pow((boundaryThree.y - boundaryOne.y), 2);
  secondBoundary = pow((boundaryTwo.x - boundaryOne.x), 2) +
                   pow((boundaryTwo.y - boundaryOne.y), 2);

  float ptFirstBoundProj =
      ((location.x - boundaryOne.x) * (boundaryThree.x - boundaryOne.x)) +
      ((location.y - boundaryOne.y) * (boundaryThree.y - boundaryOne.y));
  float ptSecondBoundProj =
      ((location.x - boundaryOne.x) * (boundaryTwo.x - boundaryOne.x)) +
      ((location.y - boundaryOne.y) * (boundaryTwo.y - boundaryOne.y));

  if (((0 < ptSecondBoundProj) && (ptSecondBoundProj < secondBoundary)) ||
      ((0 < ptFirstBoundProj) && (ptFirstBoundProj < firstBoundary))) {
    return true;
  } else {
    return false;
  }
