...
void Environment::envInit()
{
    for(int i = 0; i < environmentHeight; i++)
    {
        ...
        {
            this->patches[i][j].color.r = this->patches[i][j].color.g = this->patches[i][j].color.b = this->patches[i][j].color.a = 1.0f;
        ...
        for(int j = 0;j < environmentWidth; j++)
        {
            for(int k = i-1; k <= i+1; k++)
            {
                for(int l = j-1; l <= j+1; l++)
                {
                    ...
                    {
                        this->patches[k][l].foodTrailChemical += (this->patches[i][j].foodTrailChemical * (diffusionRate/8));
                    }
                }
            }
        }
...
Patch Environment::getPatch(int i, int j)
{
