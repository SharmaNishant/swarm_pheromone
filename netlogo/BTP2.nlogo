patches-own [
  ...
  deathSmell           ;; amount of chemical of death smell on this patch
  lifeRisk             ;; amount of risk around food patches
...
turtles-own [
  dead?                ;; check if the turtle is dead or not
  deathTick
  typeBot
]
...
to setup-food  ;; patch procedure
  ...
  [ set food one-of [1 2] ]
  [ set lifeRisk 0 ] ;; 0 risk at no food places
end
...
to recolor-patch  ;; patch procedure
  ...
  [ ifelse food > 0
    ...
    [ ifelse deathSmell < 1 
      ...
      [ set pcolor scale-color red deathSmell 1 5 ] ] ]
end
...
to go  ;; forever button
  ...
  [ if who >= ticks [ stop ] ;; delay initial departure
    ...
    [ 
      ...
      [
        ifelse dead? = true 
        [ set deathSmell deathSmell + 20
          if deathTick + 50 < ticks
          [ set deathTick ticks
            wiggle
            fd 1
             ]
...
;;          show "death"
    ...
    [
     ...
     check-death-smell
    ifelse color = red
    [ ifelse (deathSmell < 1) [ look-for-food] [downhill-deathSmell]  ]       ;; not carrying food? look for it
    [ ifelse (deathSmell < 1) [ return-to-nest ] [downhill-deathSmell]  ]     ;; carrying food? take it back to nest
    wiggle
    fd 1 ]
  ]
  diffuse chemical (diffusion-rate / 100)
  diffuse deathSmell (death-Chemical-Diffusion-Rate / 100)
  ask patches
  [ set chemical chemical * (100 - evaporation-rate) / 100  ;; slowly evaporate chemical
    set deathSmell deathSmell * (100 - death-Chemical-Evaporation-Rate) / 100
    if food > 0 [set deathSmell 0]
    recolor-patch ]
  tick
end

to check-death-smell
  if (deathSmell > 2)
  [ uphill-nest-scent 
    stop]
  if (deathSmell > 0.5)
...
to simulate-death
  if lifeRisk > 0
  [
    let robotLifeRisk random 100
    ;;set robotLifeRisk robotLifeRisk / 100
    ;;show robotLifeRisk
    if robotLifeRisk < lifeRisk
    [ set dead? true
      set color yellow
      set deathTick ticks
      set deathSmell deathSmell + 60 
...
to not-look-for-food  ;; turtle procedure
  if food > 0
  [ back 5 ] ;;5 steps back 
end
...
to downhill-deathSmell  ;; turtle procedure
  let scent-ahead nest-scent-at-angle   0
  let scent-right nest-scent-at-angle  45
  let scent-left  nest-scent-at-angle -45
  ifelse (scent-right > scent-ahead) and (scent-left > scent-ahead)
  [ ]
...
to-report deathSmell-at-angle [angle]
  let p patch-right-and-ahead angle 1
  if p = nobody [ report 0 ]
  report [deathSmell] of p
end
...
"ants" 1.0 0 -7500403 true "" "plotxy ticks sum [turtles] of turtles with [pcolor = red]"
...
1317
183
death-Chemical-Evaporation-Rate
death-Chemical-Evaporation-Rate
0
100
5
